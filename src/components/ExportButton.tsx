  ...
  return (
    <button
      ...
      className="
        ...
        rounded-2xl sm:rounded-3xl px-6 sm:px-8 py-3 sm:py-4 shadow-lg hover:shadow-xl 
        transform hover:scale-105 active:scale-95 transition-all duration-300
        disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:scale-100
        flex items-center gap-3 sm:gap-4 text-base sm:text-lg
      "
    >
      <span className="text-xl sm:text-2xl">📄</span>
      <span>Export Session</span>
      {messages.length > 0 && (
        <span className="bg-white/20 px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-bold">
          {messages.length}
        </span>
      )}
    </button>
